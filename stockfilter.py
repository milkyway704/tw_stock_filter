...
def get_canslim_info(ticker):
    try:
        ...
        if not q_financials.empty and "Net Income" in q_financials.index:
            # 確保數據清理掉 NaN
            ni = q_financials.loc["Net Income"].dropna()
            
            # 迴圈計算 4 個季度的 YoY (i=0 是最新，i=4 是去年同期)
            # 要算 4 個 YoY，我們至少需要 4+4=8 季數據
            for i in range(4):
                try:
                    if len(ni) >= (i + 5):
                        curr_q = ni.iloc[i]
                        last_y_q = ni.iloc[i + 4]
                        if pd.notna(curr_q) and pd.notna(last_y_q) and last_y_q != 0:
                            growth = ((curr_q / last_y_q) - 1) * 100
                            a_list.append(growth)
                        else:
                            a_list.append(0.0)
                    else:
                        a_list.append(None) # 數據不足
                except:
                    a_list.append(None)
            
            # C 指標：a_list 的第 1 個 (最新一季)
            eps_growth = a_list[0] if (a_list and a_list[0] is not None) else 0.0
        
        # 備案：如果報表真的抓不到 C，嘗試從 info 抓
        if eps_growth == 0:
        ...
        return {
            ...
            "eps_growth": eps_growth,            # C
            "a_list": a_list,                    # A (List 格式)
                ...
                with st.spinner(f'正在讀取 {selected_stock} 財務數據...'):
                    ...
                    if data:
                        ...
                        with m1:
                            ...
                            # A 指標：改為序列顯示
                            st.write("**A: 近四季每季 YoY**")
                            if data['a_list']:
                                # 將數據轉為顯示字串
                                a_str_list = [f"{v:.1f}%" if v is not None else "N/A" for v in data['a_list']]
                                # 用箭頭展示趨勢（最新 ← 最舊）
                                st.subheader(f"{' ← '.join(a_str_list)}")
                                # 計算平均供參考
                                valid_vals = [v for v in data['a_list'] if v is not None]
                                if valid_vals:
                                    avg_a = sum(valid_vals) / len(valid_vals)
                                    st.caption(f"近四季平均成長：{avg_a:.1f}%")
                            else:
                                st.caption("數據不足")
                        ...
                        if is_strong:
                            ...
                            st.warning(f"⚠️ **{selected_stock} 診斷提醒：** 建議搭配 A 指標趨勢觀察。")
                    else:
